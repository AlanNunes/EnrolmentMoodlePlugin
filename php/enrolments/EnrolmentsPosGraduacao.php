...
Class EnrolmentsPosGraduacao extends Enrolments{
  ...
  public function getStudentsNotSubscribedInAnyCourse(){
    $sql = "select distinct username, lastname, firstname, email from mdl_user u where id not in (select userid from mdl_user_enrolments) AND u.lastname LIKE '%POS-EAD%'";
    $result = $this->conn->query($sql);
    if($result->num_rows > 0){
